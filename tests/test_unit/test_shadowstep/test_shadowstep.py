...
class TestShadowstep:
                ...
                with patch('pathlib.Path.is_dir', return_value=True):
                    with patch('pathlib.Path.name', 'valid'):
                        with patch('os.walk', return_value=[('/valid/path', [], ['page_test.py'])]):
                            with patch('pathlib.Path.relative_to', return_value=Mock(parts=['page_test'])):
                                with patch('pathlib.Path.with_suffix', return_value=Mock(parts=['page_test'])):
                                    with patch('importlib.import_module', return_value=Mock()):
                                        with patch.object(app, '_register_pages_from_module'):
                                            ...
                                            with patch('pathlib.Path', side_effect=lambda x: Mock(name='__pycache__')):
                                                app._auto_discover_pages()
                                                # Should not raise exception, just skip the path
    ...
    def test_get_page_not_found(self, app: Shadowstep):
        ...
        with pytest.raises(ValueError, match="Page 'NonExistentPage' not found in registered pages"):
